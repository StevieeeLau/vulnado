pipeline {
	...
	stages {
		stage ('Checkout') {
			steps {
				git branch:'master', url: 'https://github.com/StevieeeLau/vulnado'
			}
		}
		stage ('Build') {
			steps {
				sh '/var/jenkins_home/apache-maven-3.9.8/bin/mvn --batch-mode -V -U -e cleanverify -Dsurefire.useFile=false -Dmaven.test.failure.ignore'
			}
		}
		stage ('Analysis') {
			steps {
